...
int main( int argc, char** argv )
{
	...
	for (int other_rank = 1; other_rank < num_procs-1; other_rank++)
		...
		for(int i=0;i<height;i++)
			for(int j=0;j<block_w;j++){
				im.at<Vec3b>(i,j)[0] = image.at<Vec3b>(i,j+(other_rank-1)*block_w)[0];
				im.at<Vec3b>(i,j)[1] = image.at<Vec3b>(i,j+(other_rank-1)*block_w)[1];
				im.at<Vec3b>(i,j)[2] = image.at<Vec3b>(i,j+(other_rank-1)*block_w)[2];
		...
		MPI_Send(im.data,block_w*height*3, MPI_BYTE, other_rank, 0, MPI_COMM_WORLD);
	}
	...
	for(int i=0;i<height;i++)
		for(int j=0;j<last_w;j++){
			im.at<Vec3b>(i,j)[0] = image.at<Vec3b>(i,j+(num_procs-2)*block_w)[0];
			im.at<Vec3b>(i,j)[1] = image.at<Vec3b>(i,j+(num_procs-2)*block_w)[1];
			im.at<Vec3b>(i,j)[2] = image.at<Vec3b>(i,j+(num_procs-2)*block_w)[2];
	...
	MPI_Send(im.data,last_w*height*3, MPI_BYTE, num_procs-1, 0, MPI_COMM_WORLD);


	Mat image2(Size(height,width),type);

	for (int other_rank = 1; other_rank < num_procs-1; other_rank++)
		MPI_Recv(image2.data+block_w*(other_rank-1)*height*3,block_w*height*3, MPI_BYTE, other_rank,0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

	// last process	
	MPI_Recv(image2.data+block_w*(num_procs-2)*height*3,last_w*height*3, MPI_BYTE, num_procs-1,0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

	imwrite("color_rotation.jpg",image2);
	...
	for(int i=0; i < height;i++){
                    for(int j=0;j<width;j++){
                            image.at<Vec3b>(j,height-i-1)[0] = refer.at<Vec3b>(i,j)[0];
                            image.at<Vec3b>(j,height-i-1)[1] = refer.at<Vec3b>(i,j)[1];
			    image.at<Vec3b>(j,height-i-1)[2] = refer.at<Vec3b>(i,j)[2];
                    }
        }
